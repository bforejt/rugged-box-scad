...
### Regenerating example STLs

Each `examples/<config>/` carries a `build.json` manifest plus a
Customizer `params.json`, and the STLs are regenerated by
[`tools/render.py`](tools/render.py) (Python 3, standard library only —
no dependencies):

```sh
tools/render.py list                       # show configs and the parts each produces
tools/render.py build                      # regenerate every config (incremental)
tools/render.py build 43mm_gridfinity_4x3  # just one config
tools/render.py build --force              # rebuild unconditionally
tools/render.py snapshot --config <dir> --set "<name>"   # copy a Customizer set into a config
```

Builds are incremental: a part is re-rendered only when its STL is
older than the entrypoint `.scad`, the shared library, any included
submodule file, the params, or the manifest (discovered via OpenSCAD's
dependency output). The script finds OpenSCAD via `$OPENSCAD`, then
`openscad` on `PATH`, then the macOS app bundle, and exports **binary**
STL with `--enable predictible-output`.

To capture a new configuration: dial it in with the OpenSCAD GUI
Customizer and save a named preset (which lands in
`src/rugged-box-gridfinity.json`), then run `tools/render.py snapshot`
to copy that preset into the example folder and add a `build.json`
listing the parts. See the existing
[`examples/43mm_gridfinity_4x3/build.json`](examples/43mm_gridfinity_4x3/build.json)
for the manifest format, including how `bottom_w_handle` (a `-D`
override) and `latchx2` (a print-copies hint) are expressed.

Note: because of the boolean-tessellation noise described below,
re-rendering a boolean-heavy part (such as the Gridfinity `top`) can
shift a handful of degenerate triangles between runs; the printed
result is unchanged.
