...
// Side length of each square Gridfinity grid cell in millimeters (standard is 42)
Cell_Size = 42; // [30:0.5:50]
...
// Add feet at the bottom-back of the box bottom matching the hinge protrusion so the box can stand upright on its back when closed
Stand_On_Back = false;
...
width = Width * Cell_Size + border;
length = Length * Cell_Size + border;
bottom_height = (
...
function rb_side_rib_positions() = [
    for (j = [for (i = [0:1:Length - 1]) i * Cell_Size])
    j - (Cell_Size * (Length / 2 - 0.5))
];

function rb_rear_rib_positions() = [
    for (j = [for (i = [1:1:Width - 2]) i * Cell_Size])
    j - (Cell_Size * (Width / 2 - 0.5))
];

function rb_latch_hinge_position() = (Cell_Size * (Width / 2 - 0.5));
...
function rb_stacking_latch_positions() = (
    ...
    ? [
        ...
        for (j = [for (i = points) (i + 0.5) * Cell_Size])
        j - (Cell_Size * (Length / 2 - 0.5))
    ]
    : []
);
...
module gridfinity_baseplate(expand=false) {
    ...
    render(convexity=4)
    intersection() {
        linear_extrude(height=Cell_Size + extra_depth)
...
module gridfinity_top_base_strip(i) {
    ...
    if (trim > 0) {
        ...
        intersection() {
            _strip();
            translate([0, hx * Cell_Size / 2, 0])
            cube([Cell_Size * (Width + 1), Cell_Size, Cell_Size], center=true);
        }
    } else {
        _strip();
    }
...
module main() {
    ...
    rbox_size_adjustments(
        ...
        third_hinge_width=Third_Hinge ? (Cell_Size * 5) : 0,
